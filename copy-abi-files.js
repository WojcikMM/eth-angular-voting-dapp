...
const outputPath = './src/app/web3/abi-files/';
const contractFileExtension = 'json'

function execute(contract) {
    const fileContent = fs.readFileSync(`${inputPath}${contract.input}.${contractFileExtension}`, {encoding: 'utf-8'});
    if (!fileContent) {
    ...
    if (!json.hasOwnProperty('abi')) {
        throw new Error(`Cannot find abi property in file ${contract.input} content.`);
    }
